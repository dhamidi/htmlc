...
/*
 * Visually hidden — never display:none, which would drop the native select
 * from focus/keyboard reach and the accessibility tree entirely, breaking
 * both the zero-JS keyboard/mouse story and screen-reader access to the
 * real control. Identical technique to Checkbox.vue's/RadioGroup.vue's own
 * hidden inputs. Deliberately NOT paired with a click-through label the way
 * those two files' decorative boxes are: this component's real interactive
 * unit already has its own separate, fully-operable custom trigger button
 * below (see this file's header comment, "Two real, independently-operable
 * controls" — the native select and the custom trigger are two distinct
 * stops, by design, not one input driving one paired visual).
 */
.radix-select-native {
  position: absolute;
  opacity: 0;
